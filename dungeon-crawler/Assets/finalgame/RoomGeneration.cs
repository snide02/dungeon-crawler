    ...
    public class RoomGeneration : MonoBehaviour
    {
        ...
        public void printWalls(int gridx, int gridY, RoomType type)
        {
            ...
            for (int x = 1; x <= RoomWidth; x += 1)
            {
                ...
                GameObject top = setWorldGrid(Grid, Instantiate(wallPrefab, Grid.CellToLocal(new Vector3Int(gridx + x, gridY, 0)), Quaternion.identity));
                GameObject bottom = setWorldGrid(Grid,Instantiate(wallPrefab, Grid.CellToLocal(new Vector3Int(gridx + x, gridY + RoomWidth + 1, 0)), Quaternion.identity));
            ...
            for (int y = 1; y <= RoomWidth; y += 1)
            {
                ...
                GameObject left = setWorldGrid(Grid, Instantiate(wallPrefab, Grid.CellToLocal(new Vector3Int(gridx, gridY + y, 0)), Quaternion.identity));
                GameObject right = setWorldGrid(Grid, Instantiate(wallPrefab, Grid.CellToLocal(new Vector3Int(gridx + RoomWidth + 1, gridY + y, 0)), Quaternion.identity));
                ColorWall(left, type);
            ...
            GameObject topleft = setWorldGrid(Grid, Instantiate(wallPrefab, Grid.CellToLocal(new Vector3Int(gridx, gridY, 0)), Quaternion.identity));//bottom left
            GameObject topRight = setWorldGrid(Grid, Instantiate(wallPrefab, Grid.CellToLocal(new Vector3Int(gridx, gridY + RoomWidth + 1, 0)), Quaternion.identity));
            GameObject bottomleft = setWorldGrid(Grid, Instantiate(wallPrefab, Grid.CellToLocal(new Vector3Int(gridx + RoomWidth + 1, gridY, 0)), Quaternion.identity));
            GameObject bottomRight = setWorldGrid(Grid, Instantiate(wallPrefab, Grid.CellToLocal(new Vector3Int(gridx + RoomWidth + 1, gridY + RoomWidth + 1, 0)), Quaternion.identity));
            ...
            {
                setWorldGrid(Grid, Instantiate(Enemy1, Grid.CellToLocal(new Vector3Int(gridx + (Random.Range(1, RoomWidth)), gridY + (Random.Range(1, RoomWidth)), 0)), Quaternion.identity));

            }

            if (type == RoomType.MEDIUM)
            {
                setWorldGrid(Grid, Instantiate(Enemy2, Grid.CellToLocal(new Vector3Int(gridx + (Random.Range(1, RoomWidth)), gridY + (Random.Range(1, RoomWidth)), 0)), Quaternion.identity));
                setWorldGrid(Grid, Instantiate(Enemy2, Grid.CellToLocal(new Vector3Int(gridx + (Random.Range(1, RoomWidth)), gridY + (Random.Range(1, RoomWidth)), 0)), Quaternion.identity));
            ...
            {
                daboss = setWorldGrid(Grid, Instantiate(boss, Grid.CellToLocal(new Vector3Int(gridx +1+ (Random.Range(RoomWidth/2, RoomWidth/2)), gridY +1+ (Random.Range(RoomWidth / 2, RoomWidth / 2)), 0)), Quaternion.identity));
        ...
        public void placeDoors(IDictionary<Vector2Int, List<Vector2Int>> dictionary)
        {
            ...
            foreach (Vector2Int room in dictionary.Keys)
            {
                ...
                foreach (Vector2Int adjacentRoom in adjacent)
                {
                    ...
                    GameObject door = setWorldGrid(Grid, Instantiate(doorPrefab, doorWorldCoord, Quaternion.identity));
        ...
        void teleportPlayer(DoorGenerationSettings settings, Transform player)
        {
            ...
            (int gridx, int gridy) = convertRoomToGrid(settings.StartRoom.x, settings.StartRoom.y);
             Debug.Log("RoomGen Teleport player to gird  " + gridx + " = " + gridy);
            int radius = 1 + RoomWidth / 2;
            int centerx = gridx + radius;
            int centery = gridy + radius;
            ...
            Debug.Log("RoomGen Teleport player to " + teleportLocation);

            player.position = teleportLocation;
        }

        private GameObject setWorldGrid(Grid grid, GameObject obj) {
            var occupant = obj.GetComponent<GridOccupant>();
            if (occupant != null) {
